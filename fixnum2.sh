#!/bin/bash
set -e
cp /opt/teoria/server.js /opt/teoria/server.js.bak7 2>/dev/null || true
...
if(!getS("remind_on")) setS("remind_on","0");
if(!getS("remind_msg")) setS("remind_msg","وينك يا {name}؟ 👋\nصرلك {days} أيام ما فتحت TeoriaAI — امتحان التوريا ما بيستنى 🚗\nكمّل من وين ما وقفت: https://teoriaai.com");
...
// ================= واتساب =================
let sock=null, qrData=null, waStatus="starting", pairCode=null, pairWanted=null;
async function startWA(){
  ...
  // طلب كود ربط برقم الهاتف (بديل QR)
  if(pairWanted && !state.creds.registered){
    setTimeout(async ()=>{
      try{ pairCode = await sock.requestPairingCode(pairWanted); console.log("PAIR CODE:", pairCode); }
  ...
  sock.ev.on("connection.update", async (u)=>{
    ...
    if(connection==="open"){ qrData=null; pairCode=null; pairWanted=null; waStatus="connected"; ev("wa","اتصل الواتساب ✅"); }
    if(connection==="close"){
...
function candidates(raw){
  ...
  if(p.startsWith("970")||p.startsWith("972")) p = p.slice(3);
  else if(p.startsWith("0")) p = p.slice(1);
  return ["970"+p, "972"+p];
}
async function resolveJid(raw){
  // واتساب هو المرجع: منجرب المقدمتين ومنعتمد اللي بيقول إنه موجود فعلاً
...
app.post("/api/enter", async (req,res)=>{
  try{
    ...
    if(s && s.banned) return res.json({ok:0, err:"هذا الرقم محظور — تواصل مع المدرب"});
    if(s && s.active){
      // مسجّل ومفعّل — فوت فوراً بنفس الحساب
      let token = s.token;
      if(!token){ token = crypto.randomBytes(24).toString("hex");
        db.prepare("UPDATE students SET token=? WHERE id=?").run(token, s.id); }
      db.prepare("UPDATE students SET last_seen=? WHERE id=?").run(Date.now(), s.id);
      ev("enter", s.name+" فات بحسابه (جهاز/زيارة)");
      return res.json({ok:1, token, name:s.name});
    }
    // جديد أو مسجّل بس ما فعّل — كود واتساب
...
app.post("/api/login", async (req,res)=>{
  try{
    ...
    if(rate[key].length>=3) return res.json({ok:0, err:"جربت كثير — استنى ساعة"});
    const code = String(Math.floor(100000+Math.random()*900000));
    db.prepare("UPDATE students SET code=?, code_exp=?, attempts=0 WHERE id=?").run(code, Date.now()+15*60e3, s.id);
    await sock.sendMessage(s.jid, { text: tmpl(getS("code_msg"),{name:s.name,code}) });
    rate[key].push(Date.now());
    ev("login", s.name+" طلب كود دخول من جهاز جديد");
    res.json({ok:1, name:s.name});
  }catch(e){ console.log(e); res.json({ok:0, err:"صار خطأ — جرب كمان مرة"}); }
});
...
app.post("/api/verify", async (req,res)=>{
  ...
  res.json({ok:1, token, name:s.name});
});
...
app.get("/siwad/api/state", guard, (req,res)=>{
  ...
  res.json({ wa:waStatus, qr:qrData, stats, chart, srv:{mem,disk},
    ...
    msgs:{code_msg:getS("code_msg"), welcome_msg:getS("welcome_msg"), remind_msg:getS("remind_msg")}, remind_on:getS("remind_on") });
});
...
app.post("/siwad/api/settings", guard,(req,res)=>{
  ...
  if(b.remind_on!==undefined) setS("remind_on", b.remind_on?"1":"0");
  if(b.remind_msg!==undefined) setS("remind_msg", String(b.remind_msg).slice(0,600));
...
app.post("/siwad/api/paircode", guard, async (req,res)=>{
  try{
    let p=String(req.body.phone||"").replace(/\D/g,"");
    if(p.startsWith("00")) p=p.slice(2);
    if(p.startsWith("0")) p="970"+p.slice(1);
    if(!/^97[02]\d{8,9}$/.test(p)) return res.json({ok:0, err:"رقم مش مظبوط — مثال: 0569999478"});
    pairWanted=p; pairCode=null;
    try{ await sock.logout(); }catch(e){}
    execSync("rm -rf /opt/teoria/wa_auth");
    waStatus="starting"; qrData=null; startWA();
    ev("wa","طلب كود ربط برقم "+p);
    res.json({ok:1});
  }catch(e){ res.json({ok:0, err:"صار خطأ"}); }
});
app.get("/siwad/api/paircode", guard, (req,res)=>{
  res.json({ok:1, code:pairCode, wa:waStatus});
});
...
<div id="t-se" style="display:none">
  ...
  <div class="box"><h3>التذكير التلقائي 🔔</h3>
    <label class="switch"><input type="checkbox" id="remOn" style="width:22px;height:22px" onchange="saveRem()"> ذكّر الطالب اللي غايب 3+ أيام (رسالة واتساب يومية الساعة 6 المسا، بحد أقصى 20 طالب باليوم)</label>
    <span class="lbl">نص التذكير ({name} و {days})</span><textarea id="remTx"></textarea>
    <button class="btn" style="margin-top:10px" onclick="saveRem(1)">💾 احفظ</button>
    <div class="ok-t" id="remMsg"></div>
  </div>
  ...
  <div class="box"><h3>الواتساب</h3>
    <div class="row"><input id="pcPhone" placeholder="رقم الواتساب: 0569999478" style="flex:1;min-width:150px;direction:ltr;text-align:left">
    <button class="btn" onclick="pairPhone()">🔑 اربط بكود (بدون QR)</button></div>
    <div id="pcOut" style="margin-top:10px;font-size:22px;font-weight:900;letter-spacing:4px;direction:ltr;text-align:center"></div>
    <div class="hint">اكتب رقمك واضغط — بيطلعلك كود من 8 خانات. بواتساب: الإعدادات ← الأجهزة المرتبطة ← ربط جهاز ← <b>الربط برقم الهاتف بدلاً من ذلك</b> ← أدخل الكود. (مناسب لما تكون فاتح اللوحة من نفس التلفون)</div>
    <hr style="border:none;border-top:1px solid var(--ln);margin:14px 0">
    <button class="btn ghost" onclick="relink()">🔄 إعادة ربط بـQR (لو عندك جهاز ثاني)</button>
  </div>
...
async function load(){
  ...
  $("#remOn").checked=D.remind_on==="1";
  if(document.activeElement!==$("#remTx")) $("#remTx").value=D.msgs.remind_msg||"";
...
function render(){
  ...
  $("#list").innerHTML=L.map(function(s){
   var st=s.banned?'<span class="st p">🚫 محظور</span>':'<span class="st '+(s.active?"a":"p")+'">'+(s.active?"مفعّل":"غير مفعّل")+'</span>';
   var pg=s.prog?('<div class="meta">📈 '+s.prog.n+' امتحان · أفضل '+s.prog.best+'% · معدل '+s.prog.avg+'%</div>'):'<div class="meta">📈 لسا ما حل امتحانات</div>';
   return '<div class="stu"><div>'+st+
   '<div class="nm">'+esc(s.name)+'</div><span class="ph">0'+s.phone+'</span>'+
   '<div class="meta">سجّل '+ago(s.created)+' · آخر ظهور: '+ago(s.last_seen)+'</div>'+pg+'</div>'+
   '<div class="acts">'+
   '<button class="btn sm '+(s.active?"r":"g")+'" data-act="tg" data-id="'+s.id+'">'+(s.active?"أوقف":"فعّل")+'</button>'+
   '<button class="btn sm ghost" data-act="prog" data-id="'+s.id+'" data-nm="'+esc(s.name)+'">📈 تفاصيل</button>'+
   '<button class="btn sm ghost" data-act="pm" data-id="'+s.id+'" data-nm="'+esc(s.name)+'">💬 رسالة</button>'+
   '<a class="btn sm ghost" style="text-decoration:none" target="_blank" href="https://wa.me/972'+s.phone+'">واتساب</a>'+
   '<button class="btn sm '+(s.banned?"g":"r")+'" data-act="ban" data-id="'+s.id+'" data-nm="'+esc(s.name)+'" data-b="'+(s.banned?1:0)+'">'+(s.banned?"فك الحظر":"🚫 احظر")+'</button>'+
   '<button class="btn sm ghost" data-act="del" data-id="'+s.id+'" data-nm="'+esc(s.name)+'">🗑️</button>'+
   '</div></div>';}).join("")||'<div class="hint" style="text-align:center;padding:20px">ما في نتائج</div>';
  document.querySelectorAll("#list [data-act]").forEach(function(b){ b.onclick=function(){
    var id=+b.dataset.id, nm=b.dataset.nm||"", a=b.dataset.act;
    if(a==="tg")tg(id); else if(a==="prog")prog(id,nm); else if(a==="pm")pm(id,nm);
    else if(a==="ban")ban(id,nm,+b.dataset.b); else if(a==="del")del(id,nm);
  };});
}
...
async function prog(id,nm){const r=await api("prog?id="+id);
  if(!r.ok||!r.rows||!r.rows.length){alert(nm+": ما في امتحانات مسجلة بعد");return;}
  var NL=String.fromCharCode(10);
  alert("📈 آخر امتحانات "+nm+":"+NL+NL+r.rows.map(function(x){
   var p=Math.round(x.correct*100/x.total);
...
async function saveRem(save){ await api("settings",{remind_on:$("#remOn").checked, remind_msg:$("#remTx").value});
  if(save){$("#remMsg").textContent="انحفظ ✅";setTimeout(()=>$("#remMsg").textContent="",2000);} }
...
async function pairPhone(){
  const ph=$("#pcPhone").value.trim();
  if(!ph){alert("اكتب رقمك");return;}
  if(!confirm("رح ينفصل أي ربط حالي ويطلع كود جديد — أكيد؟"))return;
  const r=await api("paircode",{phone:ph});
  if(!r.ok){alert(r.err||"غلط");return;}
  $("#pcOut").textContent="⏳ جاري توليد الكود…";
  clearInterval(PC_T);
  PC_T=setInterval(async ()=>{
    const s=await fetch("/siwad/api/paircode").then(x=>x.json()).catch(()=>null);
    if(s&&s.code){ $("#pcOut").textContent=s.code; clearInterval(PC_T); }
    if(s&&s.wa==="connected"){ $("#pcOut").textContent="✅ اتصل!"; clearInterval(PC_T); load(); }
  },2500);
}
...
// ---------- تذكير يومي للطلاب النايمين ----------
async function dailyReminders(){
  try{
    if(getS("remind_on")!=="1" || waStatus!=="connected") return;
    const now=Date.now(), day=864e5;
    const list=db.prepare(`SELECT * FROM students WHERE active=1 AND banned=0 AND jid IS NOT NULL
      AND last_seen IS NOT NULL AND last_seen < ? AND last_seen > ?`).all(now-3*day, now-30*day);
    const already=new Set((getS("remind_sent")||"").split(","));
    const todo=list.filter(s=>!already.has(String(s.id))).slice(0,20);
    if(!todo.length) return;
    ev("remind","تذكير تلقائي لـ"+todo.length+" طالب");
    for(const s of todo){
      if(waStatus!=="connected") break;
      const days=Math.floor((now-s.last_seen)/day);
      try{ await sock.sendMessage(s.jid,{text:tmpl(getS("remind_msg"),{name:s.name,days})});
        already.add(String(s.id)); }catch(e){}
      await new Promise(r=>setTimeout(r, 20000+Math.random()*20000));
    }
    setS("remind_sent", Array.from(already).slice(-500).join(","));
  }catch(e){ console.log("remind err", e.message); }
}
// كل يوم الساعة 18:00 بتوقيت فلسطين (15:00 UTC) + تصفير سجل الإرسال أسبوعياً
setInterval(()=>{
  const d=new Date();
  if(d.getUTCHours()===15 && d.getUTCMinutes()<5) dailyReminders();
  if(d.getUTCDay()===0 && d.getUTCHours()===3 && d.getUTCMinutes()<5) setS("remind_sent","");
}, 4*60*1000);

app.post("/internal/fixjids", async (req,res)=>{
  const ip=req.socket.remoteAddress||"";
  if(!ip.includes("127.0.0.1") && !ip.includes("::1")) return res.status(403).end();
  if(waStatus!=="connected") return res.json({ok:0, err:"WA not connected"});
  const rows=db.prepare("SELECT id,name,phone,jid FROM students").all();
  const out=[];
  for(const s of rows){
    try{
      const jid=await resolveJid(s.phone);
      if(jid && jid!==s.jid){ db.prepare("UPDATE students SET jid=? WHERE id=?").run(jid,s.id); out.push(s.name+": "+s.jid+" -> "+jid); }
      else out.push(s.name+": "+(jid?("ok ("+jid+")"):"NOT ON WA?"));
    }catch(e){ out.push(s.name+": err"); }
    await new Promise(r=>setTimeout(r,1200));
  }
  ev("fix","إعادة فحص أرقام الطلاب من واتساب");
  res.json({ok:1, out});
});
...
echo ">>> ننتظر اتصال الواتساب (20 ثانية)..."
sleep 20
echo ">>> إعادة فحص كل الأرقام من واتساب مباشرة:"
curl -s -X POST http://127.0.0.1:3000/internal/fixjids | python3 -m json.tool --no-ensure-ascii 2>/dev/null || curl -s -X POST http://127.0.0.1:3000/internal/fixjids
echo ""
node -e "const D=require('better-sqlite3')('/opt/teoria/data.db'); console.table(D.prepare('SELECT id,name,phone,jid FROM students').all())"
echo "============================================"
echo "  ✅ الأرقام صارت بالظبط زي ما واتساب مسجلها"
echo "============================================"
