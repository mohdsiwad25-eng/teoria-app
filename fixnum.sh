#!/bin/bash
set -e
cp /opt/teoria/server.js /opt/teoria/server.js.bak6 2>/dev/null || true
...
function candidates(raw){
  ...
  // 59x / 56x = جوال/أوريدو فلسطيني ← 970 أولاً وأخيراً
  if(/^(59|56)/.test(p)) return ["970"+p, "972"+p];
  // باقي الأرقام (05x إسرائيلي) ← 972 أولاً
  return ["972"+p, "970"+p];
}
async function resolveJid(raw){
  const cands = candidates(raw);
  // للأرقام الفلسطينية: 970 مباشرة بدون سؤال واتساب (بيرد نعم عالاثنين ويلخبط)
  const digits = cands[0].slice(3);
  if(/^(59|56)/.test(digits)) return "970"+digits+"@s.whatsapp.net";
...
echo ">>> تصحيح أرقام الطلاب المخزنة (059/056 ← 970)..."
cd /opt/teoria
node -e "
const D=require('better-sqlite3')('/opt/teoria/data.db');
const rows=D.prepare('SELECT id,phone,jid FROM students').all();
let n=0;
for(const r of rows){
  if(/^(59|56)/.test(r.phone)){
    const good='970'+r.phone+'@s.whatsapp.net';
    if(r.jid!==good){ D.prepare('UPDATE students SET jid=? WHERE id=?').run(good,r.id); n++; }
  }
}
console.log('صُححت '+n+' أرقام');
console.table(D.prepare('SELECT id,name,phone,jid,active FROM students').all());
"
pm2 restart teoria --update-env
sleep 3
echo ""
echo "============================================"
echo "  ✅ انصلحت المقدمة — جرّب دخول برقمك هلأ"
echo "============================================"
