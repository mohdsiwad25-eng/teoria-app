...
set -e
echo ">>> نسخة احتياطية..."
cp /opt/teoria/server.js /opt/teoria/server.js.bak2 2>/dev/null || true
echo ">>> تركيب النسخة المصلّحة..."
...
sleep 4
echo ">>> فحص:"
echo -n "  /api/enter: "; curl -s -X POST http://127.0.0.1:3000/api/enter -H "Content-Type: application/json" -d '{"phone":"970599999999","name":"t"}' | head -c 120; echo ""
echo -n "  /siwad: "; curl -s http://127.0.0.1:3000/siwad/api/state | head -c 40; echo ""
pm2 logs teoria --lines 3 --nostream || true
echo ""
echo "============================================"
echo "  ✅ اللوحة اتصلحت — افتح https://teoriaai.com/siwad"
echo "  سجّل خروج ودخول من جديد لو لسا فاضية"
echo "============================================"
