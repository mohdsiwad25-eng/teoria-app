...
# ===== كاش المتصفح: HTML/JSON دايماً طازة، الصور أسبوع =====
printf 'map $request_uri $tcache { default "no-cache, must-revalidate"; ~*\\.(jpg|jpeg|png|webp|svg|gif|ico|woff2?)$ "public, max-age=604800"; }\n' > /etc/nginx/conf.d/teoria-cache.conf
grep -q 'Cache-Control $tcache' /etc/nginx/sites-available/teoria || sed -i '0,/server_name teoriaai/s//server_name teoriaai/{}' /etc/nginx/sites-available/teoria
grep -q 'Cache-Control $tcache' /etc/nginx/sites-available/teoria || sed -i 's|root /var/www/teoria;|root /var/www/teoria;\n    add_header Cache-Control $tcache always;|' /etc/nginx/sites-available/teoria
...
echo ""
echo "============================================"
echo "  ✅ كل شي جاهز: https://teoriaai.com"
echo "  دخول موحد ذكي + كاش مظبوط + اللوحة /siwad"
echo "============================================"
