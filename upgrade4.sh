...
map $request_uri $tcache {
    ...
    "~*\.(jpg|jpeg|png|webp|svg|gif|ico|woff2?)$" "public, max-age=604800";
}
CACHEEOF
if ! grep -q 'Cache-Control $tcache' /etc/nginx/sites-available/teoria; then
  perl -0pi -e 's|(root /var/www/teoria;)|$1\n    add_header Cache-Control \$tcache always;|' /etc/nginx/sites-available/teoria
fi
