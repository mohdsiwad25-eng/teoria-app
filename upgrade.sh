#!/bin/bash
# ===== ترقية TeoriaAI: لوحة القيادة الاحترافية /siwad =====
set -e
...
app.get("/siwad/api/state", guard, (req,res)=>{
  const all = db.prepare("SELECT id,name,phone,active,created,last_seen,code FROM students ORDER BY id DESC").all();
...
async function load(){
  ...
  $("#kpis").innerHTML=
   ...
   kpi(s.week,"هذا الأسبوع","blue")+kpi(s.online24,"نشطين ٢٤ ساعة","green");
...
echo ">>> [3/4] تحديث بوابة الموقع (/siwad بدل /admin)..."
sed -i 's|location = /admin|location /siwad|' /etc/nginx/sites-available/teoria
...
echo ""
echo "============================================"
echo "  🛡️ لوحة القيادة جاهزة:"
echo "  https://teoriaai.com/siwad"
echo "  (كلمة السر اللي اخترتها — واتساب/طلاب/إعلانات/إعدادات)"
echo "============================================"
